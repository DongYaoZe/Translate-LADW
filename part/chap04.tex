...
{\heiti 命题 1.1}~~特征值的几何重数不能超过其代数重数。

\begin{proof}见下面的练习 1.9。\end{proof}

\subsection{1.6. 迹与行列式}

\begin{theorem}[\normalfont\heiti 定理 1.2\nopunct] 设 $A$ 是一个 $n \times n$ 矩阵，设 $\lambda_1, \lambda_2, \dots, \lambda_n$ 是它的（复数）特征值（计入重数）。那么
...
1.6. 称算子 $A$ 为\textbf{幂零}(nilpotent)的，如果 $\exists k \in \mathbb{N}_+$，使得  $A^k = \oo$ 成立。证明如果 $A$ 是幂零的，那么 $\sigma(A) = \{0\}$（即 $0$ 是 $A$ 的唯一特征值）。


1.7. 证明分块三角矩阵 $$\begin{pmatrix} A & * \\ \oo & B \end{pmatrix}$$
的特征多项式（其中 $A$ 和 $B$ 是方阵）与$\det(A - \lambda I) \det(B - \lambda I)$ 相等。（使用第三章的练习 3.11）。

1.8. 设 $\vv_1, \vv_2, \dots, \vv_n$ 是向量空间 $V$ 中的一组基。并假设基的前 $k$ 个向量 $\vv_1, \vv_2, \dots, \vv_k$ 是算子 $A$ 的特征向量，对应于特征值 $\lambda$ （即 $A\vv_j = \lambda \vv_j, j = 1, 2, \dots, k$）。证明在该基下，算子 $A$ 的矩阵具有分块三角形式 
$$\begin{pmatrix} \lambda I_k & * \\ \oo & B \end{pmatrix},$$
其中 $I_k$ 是 $k \times k$ 的单位矩阵， $B$ 是某个 $(n-k) \times (n-k)$ 矩阵。

1.9. 使用前面两个练习中的结论来证明一个特征值的几何重数不能超过其代数重数。
...
其中 $q(\lambda)$ 是一个最高为 $n-2$ 次的多项式。最后，通过比较 $\lambda^{n-1}$ 的系数来得出结论。\end{exer}
...
$$(2.1) \quad [A]_{\B\B} = \diag\{\lambda_1, \lambda_2, \dots, \lambda_n\} = \begin{pmatrix} \lambda_1 & & &{\large \oo} \\ & \lambda_2 & & \\ & & \ddots & \\ {\large \oo}& & & \lambda_n \end{pmatrix}$$
...
\begin{theorem}[\normalfont\heiti 定理 2.1\nopunct] 一个矩阵 $A$（每项的值都在 $\FF$内）可以表示为 $A = SDS^{-1}$，其中 $D$ 是一个对角矩阵，$S$ 是一个可逆矩阵（两者项中的值都在 $\FF$内），当且仅当$\FF^n$ 存在一个由 $A$ 的特征向量组成的基。
...
{\heiti 注记} ~注意，如果一个矩阵允许表示为 $A = SDS^{-1}$ 并且 $D$ 是一个对角矩阵，那么通过直接计算就可以表明，$S$ 的列是 $A$ 的特征向量，而 $D$ 的对角线元素是相应的特征值。这为定理 2.1 中相应陈述提供了另一种证明。
...
一个实数矩阵何时可以实数对角化（$A = SDS^{-1}$，其中 $S$ 和 $D$ 都是实数矩阵）的问题，实际上是一个非常简单的问题，见下面的定理 2.9。
...
\begin{theorem}[\normalfont\heiti 定理 2.2\nopunct] 设 $\lambda_1, \lambda_2, \dots, \lambda_r$ 是 $A$ 的不同特征值，设 $\vv_1, \vv_2, \dots, \vv_r$ 是相应的特征向量。那么向量 $\vv_1, \vv_2, \dots, \vv_r$ 是线性无关的。\end{theorem}
...
根据归纳假设，向量 $\vv_1, \vv_2, \dots, \vv_{r-1}$ 是线性无关的，所以 $c_k(\lambda_k - \lambda_r) = 0$ 对于 $k=1, 2, \dots, r-1$ 成立。由于 $\lambda_k \neq \lambda_r$，我们可以得出 $c_k = 0$ 对于 $k < r$成立。然后从 (2.2) 可知 $c_r = 0$，也就是说我们得到了平凡线性组合。\end{proof}

{\heiti 推论 2.3}~~如果一个算子 $A: V \to V$ 恰好有 $n = \dim V$ 个不同的特征值，那么它是可对角化的。

\begin{proof}对于每个特征值 $\lambda_k$，设 $\vv_k$ 是一个相应的特征向量（对每个特征值只选取一个特征向量）。根据定理 2.2，系统 $\{\vv_1, \vv_2, \dots, \vv_n\}$ 是线性无关的，并且由于它恰好包含 $n = \dim V$ 个向量，所以它是一组基。\end{proof}
...
{\heiti 注记2.4}~~从上述定义可以立即看出，定理 2.2 实际上表明，算子 $A$ 的特征子空间 
$$E_k := \Ker(A - \lambda_k I),\quad \lambda_k \in \sigma(A)$$ 的系统是线性无关的。

{\heiti 注记2.5}~~很容易看出，与向量基类似，子空间的系统 $\{V_1, V_2, \dots, V_p\}$ 是一组基当且仅当它既是生成集又是线性无关的。我们将此事实的证明留给读者作为练习。
...
\begin{theorem}[\normalfont\heiti 定理 2.6\nopunct] 设 $\{V_1, V_2, \dots, V_p\}$ 是一些子空间的基，并且在每个子空间 $V_k$ 中都有一组基（向量基）$\B_k$.~
\footnote{
...
\begin{lemma}[\normalfont\heiti 引理 2.7\nopunct] 设 $\{V_1, V_2, \dots, V_p\}$ 是一个线性无关的子空间族，并且在每个子空间 $V_k$ 中都有一个线性无关的向量系统 $\B_k$.~
\footnote{
...
\begin{proof}[\normalfont\heiti 定理 2.6 的证明~\nopunct]为了证明定理，我们将使用与引理 2.7 证明相同的记号，即系统 $B_k$ 由向量 $\{\bb_j : j \in \Lambda_k\}$ 组成。

引理 2.7 断言向量系统 $\{\bb_j : j = 1, 2, \dots, n\}$ 是线性无关的，所以剩下的事情就是证明这个系统是完备的。
...
\begin{theorem}[\normalfont\heiti 定理 2.8\nopunct] 设一个算子 $A: V \to V$ 恰好有 $n = \dim V$ 个特征值（计入重数）。
\footnote{
...
\begin{proof}首先，我们注意到，对于一个对角矩阵，特征值的代数重数和几何重数是相等的，因此对于可对角化算子也是如此。

现在我们来证明另一个蕴含关系。设 $\lambda_1, \lambda_2, \dots, \lambda_p$ 是 $A$ 的特征值，设 $E_k := \Ker(A - \lambda_k I)$ 是相应的特征子空间。根据注记2.4，子空间 $E_k,k=1,2,\dots,p$ 是线性无关的。
...
下面的定理实际上已经证明过了（它本质上是定理 2.8 在实数空间上的情况）。我们在此陈述只是为了总结实数矩阵可实数对角化的情形。

\begin{theorem}[\normalfont\heiti 定理 2.9\nopunct] 一个 $n \times n$ 实数矩阵 $A$ 允许实数分解（即表示为 $A = SDS^{-1}$，其中 $S$ 和 $D$ 是实数矩阵，$D$ 是对角矩阵且 $S$ 可逆）当且仅当它允许复数分解并且 $A$ 的所有特征值都是实数。\end{theorem}
...
还有一个不使用定理 2.8 的解释。即，我们得到的特征子空间 $\Ker(A - I)$ 是一维的（由向量 $(1, 0)^T$ 张成）。如果 $A$ 是可对角化的，那么它将在某组基下具有对角形式 $\begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix}$，
\footnote{
...
2.2. 设 $A$ 是一个实数方阵，$\lambda$ 是它的一个复数特征值。假设 $\vv = (v_1, v_2, \dots, v_n)^T$ 是一个相应的特征向量，$A\vv = \lambda \vv$.~证明 $\overline{\lambda}$ 是 $A$ 的一个特征值，并且 $\overline{\vv}$ 是 $A$ 的相应特征向量。这里 $\overline{\vv}$ 是向量 $\vv$ 的复共轭，$\overline{\vv} := (\overline{v}_1, \overline{v}_2, \dots, \overline{v}_n)^T$.~
...
2.7. 对下面的矩阵进行对角化：$$\begin{pmatrix} 2 & 0 & 6 \\ 0 & 2 & 4 \\ 0 & 0 & 4 \end{pmatrix}$$
...
2.10. 设 $A$ 是一个 $5 \times 5$ 矩阵，且仅有 3 个特征值（不计重数）。假设我们知道其中一个特征子空间是三维的。
你能说 $A$ 是否可对角化吗？

2.11. 给出一个 $3 \times 3$ 矩阵的例子，它不能被对角化。在构造了矩阵之后，你能使它“一般”一些，使得矩阵的特殊结构不明显吗？

2.12. 设一个非零矩阵 $A$ 满足 $A^5 = 0$.~证明 $A$ 不能被对角化。更一般地说，任何非零幂零矩阵，即满足$\exists N \in \mathbb{N}_+$，使得  $A^N = \oo$ 成立的矩阵，都不能被对角化。
