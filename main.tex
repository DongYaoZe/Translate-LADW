...
\date{\today}  % 日期
\version{10.0.0-dev}  % 同步 Linear Algebra Done Wrong 2026-04-30 版
 % 其他信息
...
% 译者补充的教学路标与版本校勘信息采用不同层级：
% 阅读提示服务于第一次学习；校勘注仅记录英文恢复文本的修正依据。
\newcommand{\readingtip}[1]{%
  \par\smallskip\noindent{\small\heiti 译者阅读提示：}{\small #1}\par\smallskip}
\newcommand{\proofreadnote}[1]{%
  \footnote{\textcolor{gray}{\textit{校勘注：}#1}}}
...
% 历史草稿/旧题解版本已归档至 legacy/drafts，不参与正式构建。
\input{part/exercises}
...
%%% 附录 %%%%%%%
% 旧 appendix 草稿已归档至 legacy/drafts/appendix.tex；其历史图片资源不完整，故不参与正式构建。
%%%%%
\backmatter  % 结束章节自动编号
%% 索引  %%%%%%%%
\clearpage
